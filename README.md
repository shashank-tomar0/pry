...
| **Face channels** (BlazeFace over the whole frame AND over its tiles → Chrome shape detector → skin colour, `offscreen.ts`) | Faces in the captured frame, down to roughly 16 px | The model channels run first and the skin-colour pass is a SUPPLEMENT rather than a fallback: a single BlazeFace hit used to skip it entirely, which is how a page with one large portrait and a grid of thumbnails destroyed the portrait and shipped every thumbnail face. The same exclusivity survived between the two MODEL channels — Chrome's detector was asked only when BlazeFace returned nothing — so a large portrait also suppressed the one detector that might have seen a small face; it is now asked whenever a supplementary face no model box covers, and its boxes are deduped so a face is painted once. BlazeFace's short-range model has a fixed ~128×128 input, which makes the small-face miss a RESOLUTION problem rather than a threshold one: a whole 1280×800 viewport scales to 0.1, so a 44 px face arrives at ~4 px before the first convolution and no confidence tuning can recover it. The pass therefore also runs over overlapping NATIVE-RESOLUTION crops (320 px target, ≤16 tiles, drawn 1:1 so boxes come back in frame pixels, deduped across tiles), which puts that same face at ~15 px. Stitched full-page captures are deliberately not tiled: covering a 1280×10000 page would need ~130 crops, and tiling only part of it would make coverage depend on where it was cut. The skin-colour heuristic can still miss unusual lighting and add false positives on photos, a face below ~16 px is reached by no channel at all, and supplementary additions are capped at 16 per frame so a photo wall cannot blot the page. A separate DOM channel marks `img[alt~=profile|avatar|photo]` and similar as `kind: "face"`; that matches URL/alt text, so it misses most modern avatars (YouTube's `yt3.ggpht.com` images carry neither word). **There is no biometric completeness guarantee** — a missed face is not detected, and the re-OCR verifier does not look for faces at all. |
